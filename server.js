...
try {
  ...
  // Proteção: este É o servidor principal do R10 Pulse, DEVE usar porta 8080
  if (process.env.PORT && process.cwd().toLowerCase().includes('instagram-publisher')) {
    // Se estamos rodando do diretório instagram-publisher, usar porta 9000
    if (process.env.PORT === '8080') {
      delete process.env.PORT; // Remove para usar 9000 (padrão do instagram-publisher)
    }
  } else {
    // Se estamos no projeto principal R10 Pulse, FORÇAR porta 8080
...
const app = express();
// R10 Pulse principal usa porta 8080; Instagram Publisher standalone usa 9000
...
try {
  ...
  if (fs.existsSync(regularPath) && fs.existsSync(semiboldPath) && fs.existsSync(extraboldPath)) {
    ...
    EMBEDDED_FONTS_CSS = `
      <style>
  /* Fonte via CSS externo removida para evitar conflitos; usamos fontes locais/embutidas no SVG */
        @font-face {
          font-family: 'Poppins';
...
app.get('/health', async (req, res) => {
  try {
    ...
    const hasOverlayCard = fs.existsSync(overlayCard);
    const hasOverlayStory = fs.existsSync(overlayStory);
    const hasFonts = EMBEDDED_FONTS_CSS && EMBEDDED_FONTS_CSS.includes('@font-face');
    res.json({
      status: 'ok',
      port: PORT,
      overlays: { card: hasOverlayCard, story: hasOverlayStory },
      fontsEmbedded: !!hasFonts,
      env: {
        PUBLIC_BASE_URL: !!INSTAGRAM_CONFIG.PUBLIC_BASE_URL,
        IG_ACCESS_TOKEN: !!INSTAGRAM_CONFIG.ACCESS_TOKEN,
        IG_BUSINESS_ID: !!INSTAGRAM_CONFIG.BUSINESS_ID,
...
<head>
  ...
  <!-- Google Fonts: Poppins para a UI (somente na página; o SVG usa fontes embutidas) -->
...
app.post('/api/publish-carousel', async (req, res) => {
  ...
  // Converter base64 para buffer da notícia e usar SEMPRE a publi persistida
  ...
  return res.json({ success: true, postId: result.postId, carouselId: result.carouselId, mediaIds: result.mediaIds, usedCarousel: true });
  } catch (error) {
    console.error('❌ Erro ao publicar carrossel:', error);
    res.json({ success: false, error: error.message });
  }
...
app.post('/api/publish-instagram', async (req, res) => {
  ...
  try {
    ...
    if (!cardImage || !caption) {
      return res.json({ success: false, error: 'Dados do card são obrigatórios' });
    }

  // Converter base64 para buffer e PUBLICAR COMO CARROSSEL com a PUBLI PERSISTIDA
